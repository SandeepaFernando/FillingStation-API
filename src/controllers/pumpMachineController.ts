...
export const createPumpMachine = async (
  pumpName: string,
  meter: number,
  pumpOperator: number,
  tank: number
...
export const getPumpMachineById = async (id: number) => {
  ...
  if (pumpMachine) {
    ...
    return {
      status: 404,
      message: "pump machine not found",
      pumpMachine: null,
    };
  }
};

export const updatePumpMachine = async (
  id: number,
  pumpName?: string,
  meter?: number,
  pumpOperator?: number,
